...
?>  
   ...
   .innercard{
    ...
    <div class="innercard">
        ...
        <?php
            $sql = "SELECT * FROM restorder WHERE paymentStatus = '' LIMIT 1 ";
            $query = mysqli_query($con, $sql);
            if(!mysqli_fetch_array($query)){
        ?>
        <div class="center">
        <div class="alert alert-warning alert-dismissible fade show" role="alert">
        <strong>HELLO</strong> no new orders yet
        </button>
        </div>
        </div>
        <?php
...
<div class="container mt-4">
    <?php
    $sql = "SELECT * FROM restorder,restuser WHERE restorder.userId = restuser.userId  AND  paymentStatus = '' order by totalPrice desc ";
    }
    $query = mysqli_query($con, $sql);
    ...
    while($res = mysqli_fetch_array($query)){
        ...
        $costumer_email = $res['email'];$mobile = $res["userPhoneCode"].'/'.$res['userPhoneNumber'];
        $order_string = explode('-',$res["totalProducts"]);
                        ...
                        <div class="modal-dialog">
                            <div class="modal-content">
                                <div class="modal-body text-start text-black p-4">
                                    ...
                                    while ($i < count($products))
                                    {
                                        ...
                                        <div class="d-flex justify-content-between">
                                            ...
                                            <p class="text-muted mb-0"><?=$products[$i][1];?></p>
                                        </div>
                                        <?php
                                        $i += 1; 
                                    }  
                                ...
                                <div class="button-container">
                                    <a href="control/pending.php?id=<?=$res['orderId'];?>" class="button1">
                                        <span class="button1-text">Pending</span>
                                        <span class="button1-icon">&#10003;</span>
                                    </a>
                                        <a href="control/delete.php?id=<?=$res['orderId'];?>&&page=new" onclick="return confirm('Do you really want to delete the item?');" class="buttonn">
